...
class m160628_015413_notify_data extends Migration {
	...
	public function insertStatusTemplates() {
		...
		$templates = [
			...
			[ $master->id, $master->id, 'Status Invited', CoreGlobal::TPL_NOTIFY_STATUS_INVITE, null, NotifyGlobal::TYPE_NOTIFICATION, true, 'Notification triggered for respective model on requesting a new model.', null, null, 'twig', false, null, false, null, null, DateUtil::getDateTime(), DateUtil::getDateTime(), null, '{{parentType}} requested', '<b>{{parentType}}</b> - <b>{{model.displayName}}</b> has been requested for registration. {% if config.link %}Please follow this <a href="{{config.link}}">link</a>.{% endif %}{% if config.adminLink %}Please follow this <a href="{{config.adminLink}}">link</a>.{% endif %}', '{"config":{"admin":"1","user":"0","direct":"1","adminEmail":"1","userEmail":"0","directEmail":"1"}}' ],
			[ $master->id, $master->id, 'Status Invite Accepted', CoreGlobal::TPL_NOTIFY_STATUS_INVITE_ACCEPT, null, NotifyGlobal::TYPE_NOTIFICATION, true, 'Notification triggered for respective model on requesting a new model.', null, null, 'twig', false, null, false, null, null, DateUtil::getDateTime(), DateUtil::getDateTime(), null, '{{parentType}} requested', '<b>{{parentType}}</b> - <b>{{model.displayName}}</b> has been requested for registration. {% if config.link %}Please follow this <a href="{{config.link}}">link</a>.{% endif %}{% if config.adminLink %}Please follow this <a href="{{config.adminLink}}">link</a>.{% endif %}', '{"config":{"admin":"0","user":"1","direct":"1","adminEmail":"0","userEmail":"1","directEmail":"1"}}' ],
